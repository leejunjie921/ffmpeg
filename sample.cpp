...
*/
static int alloc_and_copy(AVPacket *out, const uint8_t *sps_pps,
                          uint32_t sps_pps_size, const uint8_t *in,
                          uint32_t in_size) {
  uint32_t offset =
      out->size;  // 偏移量，就是out已有数据的大小，后面再写入数据就要从偏移量处开始操作
...
int h264_extradata_to_annexb(const uint8_t *codec_extradata,
  ...
  */
  ...
  const uint8_t *extradata =
      codec_extradata +
      4;  // 扩展数据的前4位是无用的数据，直接跳过拿到真正的扩展数据
  static const uint8_t nalu_header[4] = {0, 0, 0,
                                         1};  // sps/pps数据前面的4bit的特征码
  ...
  while (unit_nb--) {  // 遍历每个sps或pps(先变量sps，然后再遍历pps)
    ...
    if (total_size >
        INT_MAX - padding) {  // total_size太大会造成数据溢出，所以要做判断
      av_log(NULL, AV_LOG_ERROR,
             "Too big extradata size, corrupted stream or invalid MP4/AVCC "
             "bitstream\n");
      av_free(out);
    ...
    // 再将sps/pps数据拷贝进out,extradata +
    // 2是因为那2字节是表示sps/pps长度的，所以要跳过
    memcpy(out + total_size - unit_size, extradata + 2, unit_size);
    // 本次sps/pps数据处理完后，指针extradata跳过本次sps/pps数据
    extradata += 2 + unit_size;
  pps:
    if (!unit_nb &&
        !sps_done++) {  // 执行到这里表明sps已经处理完了，接下来处理pps数据
      // pps的个数
      unit_nb = *extradata++;
  ...
  if (out)  // 如果out有数据，那么将out +
            // total_size后面padding(即64)个字节用0替代
...
int h264_mp4toannexb(AVFormatContext *fmt_ctx, AVPacket *in, FILE *file) {
  ...
  uint8_t
      unit_type;  // NALU头中nal_unit_type，也就是NALU类型，5表示是I帧，7表示SPS，8表示PPS
  int32_t
      nal_size;  // 一个NALU(也就是一帧，其第一个字节是头信息)的大小，它存放在NALU的前面的4个字节中
  uint8_t nal_size_len = 4;  // 存放nal_size的字节数
  uint32_t cumul_size =
      0;  // 已经处理的字节数，当cumul_size==buf_size时表示整个包的数据都处理完了
  ...
  do {
    ...
    unit_type =
        *buf & 0x1f;  // 取出NALU头信息的后面5个bit，这5bit记录NALU的类型
    ...
    if (unit_type == 5) {
      ...
      h264_extradata_to_annexb(
          ...
          &spspps_pkt, AV_INPUT_BUFFER_PADDING_SIZE);

      // 为数据添加特征码(起始码，用于分隔一帧一帧的数据)
      if ((ret = alloc_and_copy(out, spspps_pkt.data, spspps_pkt.size, buf,
                                nal_size)) < 0)
        goto fail;
    } else {
      // 非关键帧只需要添加特征码
      if ((ret = alloc_and_copy(out, NULL, 0, buf, nal_size)) < 0) goto fail;
    }
...
fail:
  av_packet_free(
      &out);  // 凡是中途处理失败退出之前都要将促使的out释放，否则会出现内存泄露
